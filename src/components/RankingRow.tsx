...
export function RankingRow({ entry }: { entry: RankingEntry }) {
  return (
    <View style={[styles.row, entry.isCurrentUser && styles.current]}>
      ...
      <UserAvatar highlighted={entry.isCurrentUser} name={entry.name} size={42} />
...
const styles = StyleSheet.create({
  row: { alignItems: 'center', backgroundColor: colors.surface, borderBottomColor: colors.border, borderBottomWidth: 1, flexDirection: 'row', gap: spacing.sm, minHeight: 72, paddingHorizontal: spacing.md, paddingVertical: 10 },
  current: { backgroundColor: colors.softGreen, borderBottomColor: colors.softGreen, borderRadius: radii.md, marginVertical: 3 },
  positionBox: { alignItems: 'center', width: 30 },
  medal: { fontSize: 21 },
  position: { color: colors.textMuted, fontFamily: typography.sans, fontSize: 14, fontWeight: '800' },
  content: { flex: 1, minWidth: 0 },
  name: { color: colors.text, fontFamily: typography.sans, fontSize: 13, fontWeight: '800' },
  subtitle: { color: colors.textMuted, fontFamily: typography.sans, fontSize: 10, marginTop: 3 },
  value: { color: colors.primary, fontFamily: typography.sans, fontSize: 14, fontWeight: '900' },
