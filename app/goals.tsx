...
import { delinquencyTone, productivityTone } from '@/utils/commercialRules';
    ...
    <ScreenContainer contentContainerStyle={styles.page} edges={['top', 'left', 'right', 'bottom']}>
      <View style={styles.mobileFrame}>
        ...
        <View style={styles.content}>
          ...
          <GoalCard badge="Senior abierto · ventas cantadas" icon="diamond-outline" insight={`Te faltan ${senior.targetValue - senior.currentValue} UF`} progress={getProgress(senior.currentValue, senior.targetValue)} title={senior.name} value={`${formatUF(senior.currentValue)} / ${formatUF(senior.targetValue)} UF`} />
          <GoalCard badge="Solo ventas emitidas" icon="ribbon-outline" insight="Faltan 80 UF emitidas para Diamante" progress={0.91} title={category.name} value="91%" />
          <GoalCard badge="Índice de productividad" icon="briefcase-outline" insight={productivity.currentValue < 1 ? 'Bajo el mínimo de 1,00' : 'Dentro del objetivo'} progress={getProgress(productivity.currentValue, productivity.targetValue)} title={productivity.name} tone={productivityTone(productivity.currentValue)} value={`${productivity.currentValue.toFixed(2)} / 1,00`} />
          ...
          <View style={styles.moraCard}>
            <View style={styles.moraTop}>
              ...
              <Text style={[styles.moraValue, { color: delinquencyTone(delinquency.currentValue) === 'red' ? colors.danger : delinquencyTone(delinquency.currentValue) === 'gold' ? colors.goldText : colors.success }]}>{delinquency.currentValue}%</Text>
            </View>
