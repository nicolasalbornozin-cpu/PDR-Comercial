...
import { ActivityIndicator, Alert, Image, ImageSourcePropType, Modal, Pressable, StyleSheet, Text, View } from 'react-native';
...
function photoSource(photo: GalleryPhoto): ImageSourcePropType {
...
export default function GalleryScreen() {
  ...
  const addPhoto = async () => {
    setUploading(true);
    try {
      if (await newsService.addGalleryPhoto('Paseo Senior 2026')) await refresh();
    } catch (cause) {
    ...
    <ScreenContainer contentContainerStyle={styles.page} edges={['top', 'left', 'right', 'bottom']}>
      ...
      <Modal animationType="fade" onRequestClose={() => setSelected(null)} transparent visible={selected !== null}>
        <SafeAreaView style={styles.modal}>
          ...
          {selected !== null && photos[selected] ? <Image resizeMode="contain" source={photoSource(photos[selected])} style={styles.fullImage} /> : null}
